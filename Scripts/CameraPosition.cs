...
public partial class CameraPosition : Camera3D
{
	...
	public override void _Process(double delta)
	{
		...
		int zoom_dir = Convert.ToInt32(Input.IsActionJustReleased("zoom_out"))
		 - Convert.ToInt32(Input.IsActionJustReleased("zoom_in"));

		_move_target += movement_direction*MoveSpeed;
		_rotate_keys_target += rotate_keys*RotateKeysSpeed;
		_zoom_target += zoom_dir*ZoomSpeed;
		...
		cam_pos.Z = Mathf.Lerp(cam_pos.Z, _zoom_target, 0.1f);
		Position = cam_pos;
	}
}
